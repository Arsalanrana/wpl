...
function myFunction() {
	...
	var n = d.getDay();
if(n==5)
    alert("Friday : Available");
else if(n==6)
alert ("Saturday : Available ");
else
...
if (mysqli_connect_errno()) {
    ...
    exit(); }
$abc="SELECT * FROM `app` where time='Dr. Arsalan Friday 03:00-06:00' OR time='Dr. Arsalan Saturday 08:00-12:00'";
...
if (mysqli_connect_errno()) {
    printf("Connect failed: %s\n", mysqli_connect_error());
    exit(); }
$abc="SELECT * FROM `doc` where id=3";
$a123 = $mysqli->query($abc);
if($a123->num_rows>0){
while($row = $a123->fetch_assoc()){
